...
void ficonStatus() {
  ...
  if (ficonFS.info(fs_info)) {
    ...
    //message += F("\",\"maxOpenFiles\":\"");
    //message += fs_info.maxOpenFiles;
    ...
    //message += F("\",\"maxPathLength\":\"");
    //message += fs_info.maxPathLength;
...
/*
    if (dstName.endsWith("/")) {
      dstName.remove(dstName.length() - 1);
    }

    if (srcName.endsWith("/")) {
      srcName.remove(srcName.length() - 1);
    }
*/
    // Переименование
...
void ficonSetup() {
  ...
  //ficonHttpServer.on("/files", HTTP_GET, ficonFiles);

  // Информация о ФС
  ficonHttpServer.on("/ficonStatus", HTTP_GET, ficonStatus);
  //ficonHttpServer.on("/status", HTTP_GET, ficonStatus);
  ...
  //ficonHttpServer.on("/httpUpload",  HTTP_POST, replyOK, ficonUpload);

  // Удаление элемента 
  //ficonHttpServer.on("/ficonDelete",  HTTP_DELETE, ficonDelete);
  ficonHttpServer.on("/ficonDelete",  HTTP_POST, ficonDelete);
  //ficonHttpServer.on("/httpDelete",  HTTP_DELETE, ficonDelete);

  // Создание / переименование / перемещение элемента
  //ficonHttpServer.on("/ficonFileCRM",  HTTP_PUT, ficonFileCRM);
  ficonHttpServer.on("/ficonFileCRM",  HTTP_POST, ficonFileCRM);
  //ficonHttpServer.on("/httpCrm",  HTTP_PUT, ficonFileCRM);
